...
namespace WebLogistica.Web.Controllers
{
    ...
    public class RetornosController : Controller
    {
			...
			public ActionResult Salientes()
      {
		...
		[CustomAuthorize(1)] //Admin
		public JsonResult Recibir(List<NameValue> jsonInput)
		{
			if (jsonInput != null)
			{
				bool res = false;
				...
				try
				{
					...
					UsuarioSessionData Usd = (UsuarioSessionData)Session["UsuarioSessionData"];
					res = Api.RecibirRetornos(Usd.Token, Le);
				}
				catch
				{
					res = false;
				...
				{
					ModelState.Clear();
					Session.Remove("RestornosListData");
					return Json(data: "Recibidos", behavior: JsonRequestBehavior.AllowGet);
				}
				else
				{
					return Json(data: "Error", behavior: JsonRequestBehavior.AllowGet);
		...
		private IList<RetornosTable> GetRetornos()
		{
			List<RetornosTable> _Retornos = new List<RetornosTable>();

			if (Session["RestornosListData"] != null)
			{
				_Retornos = (List<RetornosTable>)Session["RestornosListData"];
			}
			else
			{
				if (Session["UsuarioSessionData"] != null)
				{
					...
					UsuarioSessionData Usd = (UsuarioSessionData)Session["UsuarioSessionData"];
					var AllRetornos = Api.GetRetornos(Usd.Token);

					_Retornos = AllRetornos.AsEnumerable().Select(m => new RetornosTable()
					{
						IdEnvio = m.IdEnvio,
						DescEnvio = m.IdEnvio,
						IdCliente = m.IdCliente,
					...
					Session["RestornosListData"] = _Retornos;
				}
			}
			return _Retornos;
		}
		...
		[CustomAuthorize(1)] //Admin
		public ActionResult AjaxHandlerRetornos(jQueryDataTableParamModel param)
		{
			var allRetornos = GetRetornos();

			IEnumerable<RetornosTable> filteredRetornos;
			//Check whether the companies should be filtered by keyword
			if (!string.IsNullOrEmpty(param.sSearch))
			{
				//Used if particulare columns are filtered 
				var envioFilter = Convert.ToString(Request["sSearch_0"]);
				var clienteFilter = Convert.ToString(Request["sSearch_1"]);
				var transportistaFilter = Convert.ToString(Request["sSearch_2"]);

				//Optionally check whether the columns are searchable at all 
				var isEnvioSearchable = Convert.ToBoolean(Request["bSearchable_0"]);
				var isClienteSearchable = Convert.ToBoolean(Request["bSearchable_1"]);
				var isTransportistaSearchable = Convert.ToBoolean(Request["bSearchable_2"]);

				filteredRetornos = GetRetornos()
							.Where(c => isEnvioSearchable && c.DescEnvio.ToString().ToLower().Contains(param.sSearch.ToLower())
													||
													isClienteSearchable && c.DescCLiente.ToLower().Contains(param.sSearch.ToLower())
													||
			...
			{
				filteredRetornos = allRetornos;
			}

			var isEnvioSortable = Convert.ToBoolean(Request["bSortable_0"]);
			var isClienteSortable = Convert.ToBoolean(Request["bSortable_1"]);
			var isTransportistaSortable = Convert.ToBoolean(Request["bSortable_2"]);
			var isFecEnvioSortable = Convert.ToBoolean(Request["bSortable_3"]);
			var sortColumnIndex = Convert.ToInt32(Request["iSortCol_0"]);
			Func<RetornosTable, string> orderingFunction = (c => sortColumnIndex == 1 && isEnvioSortable ? IdxOrderIntString(c.DescEnvio.ToString()) :
																											sortColumnIndex == 2 && isClienteSortable ? c.DescCLiente :
																											sortColumnIndex == 3 && isTransportistaSortable ? c.DescTransportista :
																											sortColumnIndex == 5 && isFecEnvioSortable ? String.Format("{0:yyyyMMdd}", c.FechaEnvio) :
																											"");

			var sortDirection = Request["sSortDir_0"]; // asc or desc
			if (sortDirection == "asc")
				filteredRetornos = filteredRetornos.OrderBy(orderingFunction);
			else
				filteredRetornos = filteredRetornos.OrderByDescending(orderingFunction);

			var displayedEnvios = filteredRetornos.Skip(param.iDisplayStart).Take(param.iDisplayLength);
			var result = from c in displayedEnvios select new[] { c.IdEnvio.ToString(), c.DescCLiente, c.DescTransportista, String.Format(ConfigurationManager.AppSettings["DisplayFormatDate"].ToString(), c.FechaEnvio), c.IdEnvio.ToString() };
			return Json(new
			{
				sEcho = param.sEcho,
				iTotalRecords = allRetornos.Count(),
				iTotalDisplayRecords = filteredRetornos.Count(),
				aaData = result
			},
			JsonRequestBehavior.AllowGet);
		}
